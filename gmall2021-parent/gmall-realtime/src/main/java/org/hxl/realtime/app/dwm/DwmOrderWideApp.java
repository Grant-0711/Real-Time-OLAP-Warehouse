...
public class DwmOrderWideApp extends BaseAppV2 {
    public static void main(String[] args) {
        new DwmOrderWideApp().init(30033, 1, "DwmOrderWideApp", "DwmOrderWideApp",
        ...
        orderWideStreamWithoutDims
                .map(new RichMapFunction<OrderWide, OrderWide>() {
                    ...
                    public OrderWide map(OrderWide orderWide) throws Exception {
                        ...
                        // 2. 读取省份的信息
                        JSONObject provinceInfo = DimUtil.readDimFromPhoenix(conn, Constant.DIM_BASE_PROVINCE, orderWide.getProvince_id());
                        ...
                        // 3. 读取sku信息
                        JSONObject skuInfo = DimUtil.readDimFromPhoenix(conn, Constant.DIM_SKU_INFO, orderWide.getSku_id());
                        ...
                        // 4. 读取tm信息
                        JSONObject tmInfo = DimUtil.readDimFromPhoenix(conn, Constant.DIM_BASE_TRADEMARK, orderWide.getTm_id());
                        orderWide.setTm_name(tmInfo.getString("TM_NAME"));

                        // 5. 读取spu信息
                        JSONObject spuInfo = DimUtil.readDimFromPhoenix(conn, Constant.DIM_SPU_INFO, orderWide.getSpu_id());
